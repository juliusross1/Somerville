...
"""
Move components so their center anchors sit on the Math Axis for each layer.

Run this on a selected glyph that is built from components with a `center`
anchor. For every layer of the glyph, the script checks each component, finds
the component layer for the matching master, finds its `center` anchor, and
moves the component vertically so that anchor lands on the layer's Math Axis.

The Math Axis value is read from the associated master. The script first checks
the master's custom parameter named "Math Axis"; if that is not present, it
looks for a font metric named "Math Axis" and reads that master's metric value.

The script can also follow nested components: if the component layer does not
have a direct `center` anchor, it searches through child components and
transforms the first center anchor it finds back into the parent component's
coordinates.

Before moving a component, the script tries to disable automatic component
alignment so Glyphs does not immediately snap the component back. Components
that have no matching component layer, no usable center anchor, or no Math Axis
value are skipped and reported in the Macro panel.
"""

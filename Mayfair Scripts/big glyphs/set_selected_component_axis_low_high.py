...
"""
Set the same smart-component axis to one value on master layers and another
value on high layers.

Use this on a glyph that has the same smart component in the same component
position across its master and high layers. Select that smart component in the
current layer, run the script, choose the smart axis to edit, then enter:

- Masters: the value to apply on true font master layers.
- High layers: the value to apply on layers whose name contains "High".

The script identifies the selected component by its component index in the
current layer. It then visits every layer in the selected glyph and changes the
component at that same index when the layer is either a font master layer or a
high layer. Intermediate, brace, bracket, and other non-master/non-high layers
are skipped.

This is useful for smart components such as _smart.circle or _smart.integral
where the low/master drawings should use one smart-axis value and generated
high layers should use another value, for example height=0 on masters and
height=100 on High layers.
"""

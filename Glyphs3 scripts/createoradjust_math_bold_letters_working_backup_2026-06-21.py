#MenuTitle: Backup 2 of Create or adjust math bold letters
# -*- coding: utf-8 -*-
...
refreshes one intermediate layer with MGHT set to 900. If the source glyph has
alternate layers, the script also creates matching alternate layers plus
matching alternate MGHT=900 intermediate layers. Alternate layer rules on the
source weight axis are moved to the math-weight axis so wght does not keep
affecting the generated bold-math glyphs. It makes a temporary working copy of
the source glyph, decomposes all normal components plus corner, cap, stem,
segment, and brush helpers, and then copies the resulting outlines, anchors,
stems, and width into the target layer.
"""
...
WEIGHT_AXIS_RULE_INDEX = 2
MGHT_AXIS_RULE_INDEX = 3
SCRIPT_VERSION = "2026-06-20 alternate-rules-on-MGHT"
...
def force_layer_rules_to_mght(layer, weight_axis_index, mght_axis_index, weight_axis_id=None, mght_axis_id=None):
...
def process_bold_math_glyph(font, glyph, weight_axis_id, mght_axis_id, weight_rule_index, mght_rule_index):
    created = 0
    refreshed = 0
    ...
    for master in font.masters:
        ...
        existing_layer = matching_special_layer(font, glyph, master.id, coordinates, None, name=layer_name, has_coordinates=True)
    ...
    for source_alternate_layer in source_alternate_layers(source_glyph):
        ...
        target_axis_rules, remapped_weight_rule = remap_axis_rules_native(
            axis_rules,
            weight_rule_index,
            mght_rule_index,
            weight_axis_id,
        ...
        if master_layer is None:
            skipped += 1
            print_warning("%s: skipped alternate for %s, no target master layer" % (glyph.name, master.name))
            continue

        master_coords = master_coordinates(font, master)
        if master_coords is None:
            skipped += 1
            print_warning("%s: skipped alternate for %s, could not read master coordinates" % (glyph.name, master.name))
            continue

        low_source_master = corresponding_master_for_weight(
            font,
            master_coords,
            weight_axis_id,
            mght_axis_id,
        ...
        high_source_master = corresponding_master_for_weight(
            font,
            master_coords,
            weight_axis_id,
            mght_axis_id,
        ...
        if remapped_weight_rule:
            ...
            if stale_removed:
                print("%s: deleted %i old alternate layer(s) using wght rules before recreating with MGHT rules" % (
                    master.name,
                    stale_removed,
                ))
        ...
        alternate_source_layer = high_source_alternate if remapped_weight_rule else low_source_alternate
        if existing_alternate is None:
            ...
            if remapped_weight_rule:
                print("%s: created alternate layer %s with wght rule remapped to MGHT" % (master.name, alternate_name))
            else:
            ...
            if remapped_weight_rule:
                print("%s: refreshed alternate layer %s with wght rule remapped to MGHT" % (master.name, alternate_name))
            else:
                print("%s: refreshed alternate layer %s" % (master.name, alternate_name))
        if remapped_weight_rule:
            force_layer_rules_to_mght(
                alternate_layer,
                weight_rule_index,
                mght_rule_index,
                weight_axis_id,
        ...
        alternate_coordinates[str(mght_axis_id)] = float(TARGET_AXIS_VALUE)
        alternate_intermediate_name = "%s MGHT %s" % (alternate_name, TARGET_AXIS_VALUE)
        if remapped_weight_rule:
            stale_removed = remove_stale_special_layers(
                font,
                glyph,
            ...
            if stale_removed:
                print("%s: deleted %i old alternate intermediate layer(s) using wght rules before recreating with MGHT rules" % (
                    master.name,
                    stale_removed,
                ))
        ...
        if existing_alternate_intermediate is None:
            ...
            alternates_created += 1
            if remapped_weight_rule:
                print("%s: created alternate intermediate layer %s with wght rule remapped to MGHT at %s" % (
                    master.name,
                    alternate_intermediate_name,
                    alternate_coordinates,
                ))
            ...
            alternates_refreshed += 1
            if remapped_weight_rule:
                print("%s: refreshed alternate intermediate layer %s with wght rule remapped to MGHT at %s" % (
                    master.name,
                    alternate_intermediate_name,
                    alternate_coordinates,
                ))
...
def run_for_glyphs(font, glyphs, open_tab=False):
    ...
    try:
        for index, glyph in enumerate(glyphs, 1):
            ...
            stats = process_bold_math_glyph(
                font,
                glyph,
                weight_axis_id,
                mght_axis_id,
                WEIGHT_AXIS_RULE_INDEX,
                MGHT_AXIS_RULE_INDEX,
            )
